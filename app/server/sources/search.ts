...
import { groupKey, keywordRelevance } from "~/lib/book-match";
...
/**
 * 匹配与相关度的实现在 ~/lib/book-match：搜索页要按同一套规则在浏览器里
 * 合并各批结果，两边各写一份必然漂。这里转出去，保持既有导入路径可用。
 */
export { matchesKeyword, preciseRelevance, relevanceExact } from "~/lib/book-match";
export { keywordRelevance };
